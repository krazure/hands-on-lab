...
2. 새로운 Git 리포지터리를 만들 때, 외부에 작업 내용이 노출되지 않도록 Private Repository를 생성합니다. 이 실습 모듈에서는 BitBucket을 기준으로 설명을 진행하겠습니다.

2.1. BitBucket으로 로그인하면 아래와 같이 리포지터리 목록을 조회하는 화면이 나타나게 됩니다. 화면 좌측에 '+' 버튼을 누릅니다.

![리포지터리 조회 화면](images/2018-03-23-18-45-18.png)

2.2. 새로 나타나는 메뉴에서 `Repository` 메뉴를 클릭합니다.

![+ 버튼을 누른 후 Repository 메뉴 클릭](images/2018-03-23-18-46-22.png)

2.3. `Access Level` 은 `Private Repository`를 나타내도록 선택하고, 버전 관리 시스템은 `Git`을 선택합니다. 실습의 내용을 따르지 않고 편의에 따라 `Mercurial`을 대신 선택해도 무방하지만, 여기서는 `Git`을 기준으로 설명하겟습니다.

![신규 리포지터리 구성 방법](images/2018-03-23-18-48-03.png)

2.4. 새 리포지터리가 만들어지면 사용 방법을 설명하는 가이드가 나옵니다. 링크 하단의 체크아웃 방법을 참고하여 실제 로컬 디렉터리에 체크아웃해보도록 하겠습니다.

![리포지터리 체크아웃 방법](images/2018-03-23-18-49-18.png)

2.5. 처음 git을 설치한 경우 원격 리포지터리에 사용할 커밋한 사람에 대한 개인 정보를 지정해야만 합니다. 아래의 두 단계를 진행하도록 합니다.

    2.5.1. `git config --global user.name 사용자이름` 을 입력하여 커밋할 때 표시될 사용자 이름을 지정합니다.

    2.5.2. `git config --global user.email 메일주소` 를 입력하여 커밋할 때 표시될 사용자 메일 주소를 지정합니다.

2.6. 선호하는 명령 프롬프트 창을 열고, 앞 단계에서 만든 git 리포지터리를 체크아웃해보겠습니다.
...
2.7. 처음 리포지터리를 체크아웃하였고, 리포지터리에 파일을 제대로 추가할 수 있는지를 테스트해보기 위하여 `README.md` 파일을 새로 만들거나, 파일이 이미 있으면 텍스트 에디터로 열어 임의의 내용을 추가하고 저장합니다.

2.8. 그 다음 `git add README.md` 명령을 입력하여 Git 커밋 대상에 포함시킵니다.

2.9. 그 다음 `git commit -m "Initial commit"` 명령을 입력하여 `Initial Commit` 이라는 메시지를 포함하는 새로운 커밋을 로컬 리포지터리에 추가합니다.

2.10. 그 다음 `git push origin` 명령을 입력하여 BitBucket 리포지터리와 커밋을 동기화합니다. 작업이 완료되면 BitBucket 온라인 리포지터리에도 파일이 변경되거나 추가된 것이 잘 들어있는지 확인합니다. 아래 그림과 같이 화면이 변경되고 `README.md` 파일이 보이면 됩니다.

![변경된 리포지터리를 웹에서 조회한 모습](images/2018-03-23-21-07-15.png)
...
3.1. 작업의 편의를 위하여 `https://portal.azure.com` 웹 사이트에 로그인하여 포털을 브라우저에 띄우고, 화면 상단 우측의 Cloud Shell 아이콘을 클릭하여 창을 띄웁니다. (아래 그림 참고)
...
3.2. Cloud Shell을 처음 만들면, PowerShell과 Bash 중 어떤 것을 택할지 별도로 물어보는 대화 상자가 나타납니다. 이 때 Bash 셸을 사용하도록 선택합니다. 이 모듈에서 사용하는 모든 도구는 Win32용 어플리케이션이 제공되지만, 실습의 편의를 위하여 Bash 셸을 사용하는 것을 기준으로 설명할 것입니다.

3.3. 사용자 홈 디렉터리의 `clouddrive` 디렉터리는 Cloud Shell 생성과 동시에 초기화되는 별도의 Azure Storage의 File Storage에 마운트되어있고, 여기에 파일을 저장하면 Cloud Shell을 어떤 방식으로 띄우든 무관하게 마지막으로 저장한 내용이 그대로 유지됩니다. 이 디렉터리를 적극 활용할 것입니다.

3.4. 이제 다음과 같이 Cloud Shell 내에서 명령을 입력하겠습니다.
...
3.5. 앞 단계에서 편집한 내용이 그대로 표시되는지 확인해봅니다.
...
2. 그리고 작업의 편의를 위하여 `.gitignore` 파일을 작성합니다.
...
3. 명령 프롬프트로 로컬에 체크아웃한 디렉터리를 엽니다.
