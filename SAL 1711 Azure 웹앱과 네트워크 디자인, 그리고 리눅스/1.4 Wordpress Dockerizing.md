...
13. 왼쪽 메뉴에서 **리소스 그룹** -> sla-&lt;ID&gt; -> sal&lt;ID&gt;vm 컨테이너를 차례대로 탐색하여 들어갑니다. sal&lt;ID&gt;vm 컨테이너가 뜨면 네비게이터에서 `네트워킹`을 선택합니다. `네트워킹`을 찾기 어렵다면, 윗쪽 검색창에 `네트워킹`을 입력한 후 엔터를 입력합니다.
...
15. `인바운드 포트 규칙 추가`가 완료되면 sal&lt;ID&gt;vm 컨테이너의 네비게이터에서 `개요`를 클릭합니다.
